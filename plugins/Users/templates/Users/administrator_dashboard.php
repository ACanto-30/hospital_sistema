...
<div class="dashboard">
          ...
          <tbody>
            <?php foreach ($data as $item): ?>
              <?php if ($listType === 'users'): ?>
                ...
                <!-- =========================
                     FILA EXPANDIBLE: EDIT USER
                ========================== -->
                <tr id="edit-user-row-<?= $item->id ?>" class="edit-row" style="display:none;">
                  <td colspan="7">
                    <div class="edit-container">
                      <?= $this->Form->create(null, [
                        'url' => ['plugin' => 'Users', 'controller' => 'Users', 'action' => 'editUser', $item->id]
                      ]) ?>

                      <h4 style="margin-top:0; margin-bottom:12px;">🧩 Editar Usuario</h4>

                      <!-- Datos básicos Users -->
                      <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-bottom:15px;">
                        <div>
                          <label class="muted small">Nombre completo</label>
                          <?= $this->Form->control('full_name', ['label' => false, 'value' => $item->full_name, 'class' => 'form-input']) ?>
                        </div>
                        <div>
                          <label class="muted small">Usuario</label>
                          <?= $this->Form->control('username', ['label' => false, 'value' => $item->username, 'class' => 'form-input']) ?>
                        </div>
                      </div>

                      <div style="display:grid; grid-template-columns:2fr 1fr; gap:15px; margin-bottom:15px;">
                        <div>
                          <label class="muted small">Correo</label>
  ...
  ?>
                      ...
                      <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-bottom:15px;">
                        <div>
                          ...
                          <?= $this->Form->control('status', [
                            ...
                            'default' => $item->status ?? 'activo',
                            'class' => 'form-select'
                          ]) ?>
                        </div>

                        <div>
                          <label class="muted small">Nueva contraseña (opcional)</label>
                      ...
                      <?php if ((int)($item->role_id ?? 0) === 4): ?>
                        ...
                        <?php $assoc = $associateByUserId[$item->id] ?? null; ?>

                        <div style="display:grid; grid-template-columns:1fr 1fr 1fr; gap:15px; margin-bottom:15px;">
                          <div>
                            <label class="muted small">Cédula</label>
                            <?= $this->Form->control('associate_id_card', ['label' => false, 'value' => $assoc->id_card ?? '', 'class' => 'form-input']) ?>
                          </div>
                          <div>
                            <label class="muted small">Nombre</label>
                            <?= $this->Form->control('associate_first_name', ['label' => false, 'value' => $assoc->first_name ?? '', 'class' => 'form-input']) ?>
                          </div>
                          <div>
                            <label class="muted small">Apellido</label>
                            <?= $this->Form->control('associate_last_name', ['label' => false, 'value' => $assoc->last_name ?? '', 'class' => 'form-input']) ?>
                          </div>
                        </div>

                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:15px; margin-bottom:15px;">
                          <div>
                            <label class="muted small">Fecha nacimiento</label>
                            <?= $this->Form->control('associate_birth_date', [
                              'label' => false,
                              'type' => 'date',
                              'value' => (!empty($assoc->birth_date) && method_exists($assoc->birth_date, 'format')) ? $assoc->birth_date->format('Y-m-d') : '',
                              'class' => 'form-input'
                            ]) ?>
                          </div>
                          <div>
                            <label class="muted small">Teléfono</label>
                            <?= $this->Form->control('associate_phone', ['label' => false, 'value' => $assoc->phone ?? '', 'class' => 'form-input']) ?>
                          </div>
                        </div>
                        ...
                        <div style="display:grid; grid-template-columns:2fr 1fr; gap:15px; margin-bottom:15px;">
                          <div>
                            ...
                            <?= $this->Form->control('associate_address', ['label' => false, 'value' => $assoc->address ?? '', 'class' => 'form-input']) ?>
                          </div>
                          <div>
                            <label class="muted small">Plan</label>
                            <?= $this->Form->control('associate_plan_id', [
                              'label' => false,
                              'type' => 'select',
                              'options' => $insurancePlans,
                              'default' => $assoc->plan_id ?? null,
                              'class' => 'form-select',
                              'empty' => 'Seleccione'
                            ]) ?>
                          </div>
                      ...
                      <div style="display:flex; gap:8px; justify-content:flex-end;">
                        <?= $this->Form->button('Guardar', ['class' => 'btn-save']) ?>
                        <button type="button" class="btn-cancel" onclick="toggleEdit('edit-user-row-<?= $item->id ?>')">Cancelar</button>
                      </div>
                ...
                <tr id="edit-row-<?= $item->id ?>" class="edit-row" style="display:none;">
                  <td colspan="6">
                    <div class="edit-container">
                      ...
                      <h4 style="margin-top:0; margin-bottom:12px;">🤝 Editar Asociado</h4>
                      ...
                      <div style="display:grid; grid-template-columns:1fr 2fr; gap:15px; margin-bottom:15px;">
                        <div>
                          <label class="muted small">Cédula</label>
                          <?= $this->Form->control('id_card', ['label' => false, 'value' => $item->id_card, 'class' => 'form-input']) ?>
                        </div>

                        <div>
                          <label class="muted small">Correo electrónico</label>
                          <?= $this->Form->control('email', [
                            'label' => false,
                            'value' => ($item->user->email ?? $item->email ?? ''),
                            'class' => 'form-input',
                            'type' => 'email',
                      ...
                      <div style="display:grid; grid-template-columns:3fr 1fr; gap:15px; align-items:end;">
                        <div>
                          <label class="muted small">Motivo del cambio (opcional)</label>
                          <?= $this->Form->control('reason', [
                            'label' => false,
                            'type' => 'text',
                            'placeholder' => 'Ej: actualización de datos, solicitud del cliente, etc.',
                            'class' => 'form-input'
                          ]) ?>
                        </div>
                        <div style="display:flex; gap:5px;">
